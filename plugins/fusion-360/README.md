...
Codex / Claude Code から Autodesk Fusion 360 の Python API を通じてモデリングコマンドを送るためのローカルプラグインです。

ブリッジはあえてローカル・ファイルベースで動作します:

- アシスタントがコマンド JSON を `~/.codex/fusion360/command.json` に書き出す
...
マーケットプレイス経由でインストールした場合は、このプラグインのフォルダに `cd` してから次を実行してください (インストール先のパスはホスト (Codex / Claude Code) と環境によって異なります)。
